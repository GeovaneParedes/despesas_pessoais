#!/usr/bin/env python3
"""Análise de gastos pessoais com gráficos de resumo."""
...
def limpar_dados(df: pd.DataFrame) -> pd.DataFrame:
    """Remove linhas com valores ausentes e campos inválidos."""
    colunas_manter = ["Contas a Pagar", "Valor", "Pagamento", "Status"]
    df_limpo = df[colunas_manter].copy()

    df_limpo.dropna(
        subset=["Valor", "Pagamento", "Contas a Pagar"],
        inplace=True,
    )
    ...
    df_limpo["Data de Pagamento"] = pd.to_datetime(
        df_limpo["Pagamento"],
        errors="coerce",
    )
...
def analisar_gastos_mensais(
    df_limpo: pd.DataFrame,
) -> tuple[str, pd.DataFrame]:
    """Calcula o gasto total por mês e retorna o mês de maior despesa."""
    df_analise = df_limpo.copy()
    df_analise["MesAno"] = df_analise["Data de Pagamento"].dt.to_period("M")

    gastos_mensais = (
        df_analise.groupby("MesAno")["Valor"]
        .sum()
...
def analisar_itens_mais_caros(
    df_limpo: pd.DataFrame,
    top_n: int = 20,
) -> tuple[str, pd.DataFrame]:
    """Identifica os itens com maior gasto acumulado."""
    gastos_por_item = (
        df_limpo.groupby("Contas a Pagar")["Valor"]
        .sum()
        .sort_values(ascending=False)
        .reset_index()
    )
    gastos_por_item.rename(
        columns={"Valor": "Gasto Acumulado"},
        inplace=True,
    )
...
def gerar_e_salvar_grafico(
    ...
    """Gera e salva um gráfico de barras (horizontal ou vertical)."""
    caminho_pasta = "image"
    ...
    try:
        ...
        print(
            f"Erro: Não foi possível salvar o gráfico "
            f"em {caminho_completo}: {e}"
        )
...
def main() -> None:
    ...
    try:
        ...
        assert not df_analise.empty, (
            "ERRO: O DataFrame resultante da limpeza está vazio. "
            f"Verifique se há dados pagos em {file_name}."
        )

        maior_mes, gastos_mensais_df = analisar_gastos_mensais(df_analise)
        item_mais_caro, top_itens_df = analisar_itens_mais_caros(
            df_analise,
            top_n=20,
        )
        ...
        print(
            f"ERRO: O arquivo '{file_name}' não foi encontrado. "
